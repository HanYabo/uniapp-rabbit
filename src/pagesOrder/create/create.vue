...
import { getMemberOrderInmidateAPI, getMemberOrderPreAPI, getMemberOrderRepurchaseByIdAPI, postMemberOrderAPI } from '@/services/order'
...
const getMemberOrderPre = async () => {
  if (query.skuId && query.count) {
    const res = await getMemberOrderInmidateAPI({
      skuId: query.skuId,
      count: query.count
    })
